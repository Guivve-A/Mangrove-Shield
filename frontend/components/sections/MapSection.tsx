...
import type { LayerBundle, ScenarioConfig, UIScene, ZoneInspection } from '@/types/geospatial';
...
interface MapSectionProps {
    ...
    waterMaskGeoJson?: FeatureCollection | null;
    vulnerabilityGeoJson?: FeatureCollection | null;
    IntelligenceMap: ComponentType<IntelligenceMapProps>;
    [key: string]: unknown;
}
...
export function MapSection({
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                if (!res.ok) throw new Error('Fallo en la comunicacion con el backend.');
        ...
        void fetchData();
        const intervalId = setInterval(() => void fetchData(), POLL_INTERVAL);
    ...
    const getThreatLevel = () => {
        if (!data || data.tide.error || data.weather.error) {
            return { level: 'NORMAL' as const, color: 'text-green-400 border-green-400/50 bg-green-400/10', glow: 'shadow-[0_0_15px_rgba(74,222,128,0.2)]', pulse: false };
        }
        ...
        if ((tide > 3.0 && rain > 15) || waves > 3.5) {
            return { level: 'CRITICAL' as const, color: 'text-red-500 border-red-500/50 bg-red-500/10', glow: 'shadow-[0_0_20px_rgba(239,68,68,0.4)]', pulse: true };
        }
        if (tide > 2.5 || waves > 2.0) {
            return { level: 'WARNING' as const, color: 'text-yellow-400 border-yellow-400/50 bg-yellow-400/10', glow: 'shadow-[0_0_15px_rgba(250,204,21,0.3)]', pulse: true };
        }
        return { level: 'NORMAL' as const, color: 'text-green-400 border-green-400/50 bg-green-400/10', glow: 'shadow-[0_0_15px_rgba(74,222,128,0.2)]', pulse: false };
    };
    ...
    return (
        <section id="map" className="relative w-screen h-screen bg-[#08131d] overflow-hidden font-sans text-white">
            {/* ── IntelligenceMap: flood, mangrove, heatmap, hotspot, Deck.gl layers ── */}
                                    ...
                                    <div className="mt-1.5 grid grid-cols-2 gap-x-3 gap-y-0.5 w-full border-t border-white/5 pt-1.5">
                                        ...
                                        {data.ecosystem_health.classification && (
                                            <div className="col-span-2 mt-0.5">
                                                <span className="text-[8px] uppercase tracking-widest font-mono" style={{
                                                    color: data.ecosystem_health.classification === 'optimo' ? '#4ade80'
                                                        : data.ecosystem_health.classification === 'moderado' ? '#facc15'
                                                        : '#f87171',
                                                }}>
